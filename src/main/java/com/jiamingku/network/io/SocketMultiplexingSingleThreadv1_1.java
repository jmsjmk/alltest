...
/**
 * 其实这个版本说明了一个什么问题呢?
 * --一个多路复用器(selector)-只要发现netstat 中的发送队列不满的话，就会返回这个写事件
 * --> 只要注册了::: client.register(key.selector(), SelectionKey.OP_WRITE, buffer);
 * --> OP_WRITE 其实就是关心 send-queue 是不是有空间
 * <p>
 * 但是你又不能cancel()如果你取消了这个文件描述符，相当于在系统调用中你直接给干掉了, 再次又发送事件时候监听不到
 *
 * <p>
 * <p>
 * 但是读完之后,可以将写的事件处理掉.
        ...
        try {
            while (true) {
                ...
                if (read > 0) {
                    ...
                    System.out.println(" read ::::: 000000 ");
                    break;
                } else {
                    client.close();
                    break;
